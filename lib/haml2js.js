...
var createHaml2JsPreprocessor = function(logger, basePath, config) {
  ...
  return function(content, file, done) {
    ...
    if (moduleName) {
      ...
      done(util.format(SINGLE_MODULE_TPL, moduleName, moduleName, htmlPath, escapeContent(eval(compiled))));
    } else {
      var compiled = haml.compile(content);
      done(util.format(TEMPLATE, htmlPath, htmlPath, escapeContent(eval(compiled))));
    }
  };
};
